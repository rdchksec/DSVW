...
try:
    ...
    print(
        ...
        % ("apt-get install python-lxml"
           if os.name != "nt" else "https://pypi.python.org/pypi/lxml"))

NAME, VERSION, GITHUB, AUTHOR, LICENSE = "Damn Small Vulnerable Web (DSVW)", "1.0", "https://github.com/rdchksec/DSVW", "Author", "Unlicense (public domain)"
LISTEN_ADDRESS, LISTEN_PORT = "127.0.0.1", 8080
...
CASES = (
    ...
    ("Task 1. Source Code Disclosure<br> Try to file code file; the file is called dsvw.py", "/?path=", "/?path=dsvw.py",
    "https://www.imperva.com/resources/glossary?term=source_code_disclosure"),

    ("Task 2. Path Traversal<br>Try to find more sensitive data (for example, /etc/passwd)", "/?path=",
    "/?path=..%2F..%2F..%2F..%2F..%2F..%2Fetc%2Fpasswd" if os.name != "nt"
    else "/?path=..%5C..%5C..%5C..%5C..%5C..%5CWindows%5Cwin.ini",
    "https://www.owasp.org/index.php/Path_Traversal"),

    ("Task 3. SQL injection<br>Sign in without a password", "/login?username=&amp;password=",
    ...
    ("Task 4. Cross Site Scripting (<i>reflected</i>)<br>App version parameter seems vulnerable", "/?v=0.2",
    ...
    ("Task 5. Cross Site Scripting (<i>stored</i>)<br>Anything in comment will be stored in DB",
    ...
    ("Task 6. Cross Site Scripting (<i>DOM</i>)<br> How the task 4 is different?", "/?#lang=en",
    ...
    ("Task 7. Server Side Request Forgery<br>Query server", "/?path=",
    "/?path=http%3A%2F%2F127.0.0.1%3A631" if os.name != "nt" else
    ...
    ("Task 8. Cross Site Request Forgery<br>Click on vulnerable, click exploit and go back to vulnerable", "/?comment=",
    ...
    ("Task 9. Arbitrary Code Execution<br>Ping is OS command, you know what is pipe?", "/?domain=www.google.com",
    "/?domain=www.google.com%3B%20ifconfig"
    if os.name != "nt" else "/?domain=www.google.com%26%20ipconfig",
    "https://en.wikipedia.org/wiki/Arbitrary_code_execution"),

    ("Task 10. File Inclusion (<i>remote</i>)<br> Look what I found - https://pastebin.com/raw/6VyyNNhc", "/?include=",
    ...
    # ("Full Path Disclosure", "/?path=", "/?path=foobar",
    ...
    # ("Blind SQL Injection (<i>boolean</i>)", "?id=2",
    ...
    #  "https://www.owasp.org/index.php/Testing_for_SQL_Injection_%28OTG-INPVAL-005%29#Boolean_Exploitation_Technique"
    #  ),
    ...
    #  "https://www.owasp.org/index.php/Testing_for_SQL_Injection_%28OTG-INPVAL-005%29#Time_delay_Exploitation_technique"
    #  ),
    ...
    #  "https://www.owasp.org/index.php/Testing_for_SQL_Injection_%28OTG-INPVAL-005%29#Union_Exploitation_Technique"
    #  ),
    ...
    #  if os.name != "nt" else
    #  "/?xml=%3C!DOCTYPE%20example%20%5B%3C!ENTITY%20xxe%20SYSTEM%20%22file%3A%2F%2FC%3A%2FWindows%2Fwin.ini%22%3E%5D%3E%3Croot%3E%26xxe%3B%3C%2Froot%3E",
    ...
    #  "/?object=%s" % urllib.parse.quote(
    #      pickle.dumps(
    #          dict((_.findtext("username"),
    #                (_.findtext("name"), _.findtext("surname")))
    #               for _ in xml.etree.ElementTree.fromstring(USERS_XML).findall(
    #                   "user")))),
...
class ReqHandler(http.server.BaseHTTPRequestHandler):
    def do_GET(self):
        ...
        try:
            if path == '/':
                if "id" in params:
                    ...
                    content += "<div><span>Result(s):</span></div><table><thead><th>id</th><th>username</th><th>name</th><th>surname</th></thead>%s</table>%s" % (
                        "".join(
                            "<tr>%s</tr>" % "".join("<td>%s</td>" %
                                                    ("-" if _ is None else _)
                                                    for _ in row)
                            for row in cursor.fetchall()), HTML_POSTFIX)
                elif "v" in params:
                    content += re.sub(r"(v<b>)[^<]+(</b>)",
                ...
                #     content = str(pickle.loads(params["object"].encode()))
                    ...
                    content = (open(os.path.abspath(params["path"]), "rb")
                               if not "://" in params["path"] else
                               urllib.request.urlopen(
                                   params["path"])).read().decode()
                elif "domain" in params:
                    content = subprocess.check_output(
                        "nslookup " + params["domain"],
                        shell=True,
                        stderr=subprocess.STDOUT,
                        stdin=subprocess.PIPE).decode()
                elif "xml" in params:
                    content = lxml.etree.tostring(lxml.etree.parse(
                        io.BytesIO(params["xml"].encode()),
                        lxml.etree.XMLParser(no_network=False)),
                                                  pretty_print=True).decode()
                # elif "name" in params:
                    # found = lxml.etree.parse(io.BytesIO(
                        # USERS_XML.encode())).xpath(
                            # ".//user[name/text()='%s']" % params["name"])
                    # content += "<b>Surname:</b> %s%s" % (found[-1].find(
                        # "surname").text if found else "-", HTML_POSTFIX)
                elif "size" in params:
                    ...
                    content += "<b>Time required</b> (to 'resize image' to %dx%d): %.6f seconds%s" % (
                        int(params["size"]), int(
                            params["size"]), time.time() - start, HTML_POSTFIX)
                ...
                if HTML_PREFIX in content and HTML_POSTFIX not in content:
                    content += "<div>If nothing happens, pay more attention - URL or page elements may change<br><br><span>Attacks:</span></div>\n<ul>%s\n</ul>\n" % (
                        "".join(
                            "\n<li%s>%s - <a href=\"%s\">vulnerable</a>|<a href=\"%s\">exploit</a>|<a href=\"%s\" target=\"_blank\">info</a></li><br>"
                            %
                            (" class=\"disabled\" title=\"module 'python-lxml' not installed\""
                             if ("lxml.etree" not in sys.modules and any(
                                 _ in case[0].upper()
                                 for _ in ("XML", "XPATH"))) else "", case[0],
                ...
                content = "%s%s%s" % (
                    ...
                    params["callback"],
                    json.dumps(
                        dict((_.findtext("username"), _.findtext("surname"))
                             for _ in xml.etree.ElementTree.fromstring(
                                 USERS_XML).findall("user"))),
            ...
            content = ex.output if isinstance(
                ex, subprocess.CalledProcessError) else traceback.format_exc()
            ...
            self.end_headers()
            self.wfile.write(
                ("%s%s" % (content, HTML_POSTFIX if HTML_PREFIX in content
